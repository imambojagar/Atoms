...
class JWT {
  ...
  getUser(req: HttpRequest<any>) {
    ...
    try {
      const now = new Date();
      let data: any[]  = [token]; // JWT.parseToken(token);

      console.log('in mem userdata', data);

      return JWT.isExpired(data, now) ? null : data; // data.user;
    } catch (e) {
      return null;
    }
  }
...
export class InMemDataService implements InMemoryDbService {
  ...
  get(reqInfo: RequestInfo) {
    ...
    if (is(reqInfo, 'me')) {
      console.log("reqInfo", reqInfo.req);
  ...
  private login(reqInfo: RequestInfo) {
    ...
    return from(this.users).pipe(
      ...
      map(user => {
        ...
        console.log("api response", { headers, url, status: STATUS.OK, body: jwt.generate(currentUser) });
