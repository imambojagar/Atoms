...
export class WorkOrderViewComponent implements OnChanges {
  ...
  getWorkOrder(workOrderId: number,woType:string,idNotWO:number)
  {
    this.workOrdersService.getWorkOrder(workOrderId).subscribe(res => {
      if (res.isSuccess)
      {
        ...
        this.workOrderInfoForm.controls['reasons'].setValue(res.data.reason ? res.data.reason: '' );
        this.workOrderInfoForm.controls['faultDescription'].setValue(
          res.data.faultDescription ? res.data.faultDescription : ''
        );
