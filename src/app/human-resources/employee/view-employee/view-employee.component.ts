...
export function buildEmployeeForm(model: any) {
  let frm = new FormGroup({
    ...
    supervisorUserName: new FormControl(), // Add this line for supervisorUserName
  });

  frm.patchValue(model); // Bind the model values to the form
  return frm;
}
...
export class ViewEmployeeComponent implements OnChanges {
  ...
  Init(): void {
    ...
    this.userName = null;
    this.tabIndex = this.emp_index; //params.index;
    if (this.editData) this.userName = this.editData.data;
    // if (params.data) this.userName = params.data;
    // });
    this.getLookups();
  ...
  buildEmployeeForm() {
    ...
    if (this.userName == null && this.status == 'add') {
      this.model = {
        userName: undefined,
        joiningDate: null,
        roles: [],
  ...
  save() {
    // if (this.employeeForm.value.assetGroupsId.length>0 &&this.employeeForm.value.customersId.length>0)
    // {
    //   this.isOk=true;
    // }
  ...
  apiResponse(res: any) {
    ...
    if (sucess == true) {
      ...
      // this.route.navigate(['/employees/employee']);
